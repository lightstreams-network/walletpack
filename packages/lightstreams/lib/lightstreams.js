...
const EXPLORER = {
	"name":"Lightstreams Explorer",
	"account":"https://explorer.lightstreams.io/addr/{x}",
	"transaction":"https://explorer.lightstreams.io/tx/{x}",
	"block":"https://explorer.lightstreams.io/block/{x}"
};
...
export default class Lightstreams extends Plugin {

	constructor(){ super(Blockchains.PHT, PluginTypes.BLOCKCHAIN_SUPPORT) }
	...
	returnableAccount(account){ return { address:account.publicKey, blockchain:Blockchains.PHT }}

	contractPlaceholder(){ return '0x.....'; }
	recipientLabel(){ return "Address"; }
	...
	getEndorsedNetwork(){
		return new Network('Lightstreams Mainnet', 'https', 'node.mainnet.lightstreams.io', 443, Blockchains.PHT, '163')
	}

	isEndorsedNetwork(network){
		const endorsedNetwork = this.getEndorsedNetwork();
		return network.blockchain === Blockchains.PHT && network.chainId === endorsedNetwork.chainId;
	}
	...
	defaultToken(){ return new Token(Blockchains.PHT, 'pht', 'PHT', 'PHT', this.defaultDecimals(), '163') }
